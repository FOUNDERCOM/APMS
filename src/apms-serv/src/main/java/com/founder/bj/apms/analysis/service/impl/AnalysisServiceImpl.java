...
public class AnalysisServiceImpl implements AnalysisService {
    ...
    public Integer getDegreeNum() {
        final String sql = "SELECT count(1) FROM APMS_AUX_INFO WHERE IS_ENABLED = '1' AND AUX_EDUCATION_DEGREE > '1'";
        return ((Number) em.createNativeQuery(sql).getSingleResult()).intValue();
    }

    @Override
    public Integer getAvgSalary() {
        final String sql = "SELECT AVG(BUREAU_STD_SALARY) FROM APMS_DEPT_BUREAU WHERE IS_ENABLED = '1'";
        return ((Number) em.createNativeQuery(sql).getSingleResult()).intValue();
    }

    @Override
    public Integer getGoodPern() {
        final String sql = "SELECT COUNT(1) FROM APMS_AUX_INFO WHERE IS_ENABLED = '1' AND AUX_POLITICAL_STATUS = '1'";
        return ((Number) em.createNativeQuery(sql).getSingleResult()).intValue();
    }
