...
angular.module('WebApp').controller('CapabilityAgeCtrl', ['$rootScope', '$scope', "$listService", "$ajaxCall", function ($rootScope, $scope, $listService, $ajaxCall) {
    $ajaxCall.post({
        data : {
            controller: "AnalysisController",
            method: $rootScope["token"]['user']['org']['bureau']['id'] === '-100' ? 'getBureauAgeStatistics' : 'getStationAgeStatistics',
        ...
        success: function(res) {
            ...
            var fields = [];
            $.each(["18~25", "26~30", "31~35", "36~40", "41~45", "45+"], function(foo, field) {
                fields.push({
            ...
            var chart = AmCharts.makeChart("chart_age", {
                "type": "serial",
                "theme": "light",
                "legend": {
                    "horizontalGap": 10,
            ...
            chart.addListener("clickGraphItem", function(event) {
                ...
                $ajaxCall.post({
                    data: {
                        controller: "AnalysisController",
                        method: event.item.dataContext['type'] === 'BUREAU' ? 'getStationAgeStatistics' : 'getBureauAgeStatistics',
